...
export default function Config() {
  ...
  const disconnect = async () => {
    if (!confirm("Desligar a conta YouTube?")) return;
    setBusy(true);
    try { await api.ytDisconnect(); await load(); t("Conta desligada"); }
    catch (e: any) { t("Erro: " + e.message); }
    finally { setBusy(false); }
  };
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-800 mb-1">Configuração</h1>
      <p className="text-slate-500 text-sm mb-6">Liga a conta YouTube para criar transmissões automaticamente.</p>

      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
        <div className="flex items-center gap-2 mb-4">
          <Youtube className="h-5 w-5 text-red-600" />
          <h2 className="font-semibold text-slate-800">Conta YouTube</h2>
        </div>
        ...
        {!yt ? (
          ...
          <div className="flex items-start gap-2 p-4 rounded-xl bg-amber-50 border border-amber-200">
            ...
            <div className="text-sm text-amber-800">
              <p className="font-medium">Servidor sem credenciais Google</p>
              <p className="text-amber-700 mt-1">Define <code>YOUTUBE_CLIENT_ID</code>, <code>YOUTUBE_CLIENT_SECRET</code> e <code>YOUTUBE_REDIRECT_URI</code> no <code>.env</code> do Jetson.</p>
            </div>
          </div>
        ) : yt.is_connected ? (
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold"><CheckCircle2 className="h-3.5 w-3.5" /> Ligado</span>
              <div>
                <div className="font-semibold text-sm text-slate-800">{yt.channel_title || "(canal)"}</div>
                <div className="text-xs text-slate-400">{yt.channel_id}</div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-600">Nenhuma conta ligada.</p>
